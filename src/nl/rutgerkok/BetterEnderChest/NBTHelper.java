...
public class NBTHelper 
{
	...
	{
		Tag[] stackPropertyNBT = new Tag[5];//represents one stack, used in inventory
		...
		if(enchantments.size()>0)
		{
			...
			{
				enchantmentPropertyNBT[0] = new Tag(Tag.Type.TAG_Short,"id",(short) enchantment.getId());
				enchantmentPropertyNBT[1] = new Tag(Tag.Type.TAG_Short,"lvl",(short) enchantments.get(enchantment).shortValue());
				enchantmentsNBT[0].addTag(new Tag(Tag.Type.TAG_Compound,null,enchantmentPropertyNBT));
			}
			
			stackNBT.addTag(new Tag(Tag.Type.TAG_Compound, "tag", enchantmentsNBT.clone()));
		}
		return stackNBT;
	}
}
